...
class RelationFieldRef extends fields_1.FieldRef {
    ...
    _fetchData(instance) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(foreignDocTypeRef instanceof doc_ref_1.DocTypeRef))
            ...
            if (kind[0] === 'b') {
                ...
                switch (kind) {
                    case 'btm': {
                        ...
                        const data = yield foreignCollection.find(btmQuery, { id: true }).toArray();
                        return data.map(({ id }) => foreignDocTypeRef.instanciate({ id }));
                    }
                    case 'bto': {
                        const btoQuery = Object.assign({}, query, { [docIdKey]: (yield instance.id) });
                        const data = yield foreignCollection.findOne(btoQuery);
                ...
                switch (kind) {
                    case 'hm': {
                        ...
                        const data = yield foreignCollection.find(hmQuery, { id: true }).toArray();
                        return data.map(({ id }) => foreignDocTypeRef.instanciate({ id }));
                    }
                    case 'ho': {
                        ...
                        const { id } = yield foreignCollection.findOne(hoQuery);
